...
def system_prompt(*, now_local: str, tz_name: str) -> str:
    ...
    return (
        ...
        "- Не выдумывай шаги: только то, что пользователь явно перечислил "
        "(запятая, «и», нумерованный список). Не добавляй от себя подготовку, проверку, "
        "коммит, «разобраться» и прочий план работ. Если пунктов нет — steps=[] "
        "(один шаг подставится из title).\n"
        "Формат ответа:\n"
        ...
        "title — 2–3 конкретных слова: что именно / куда / какой предмет, не обобщение. "
        "Плохо: «Задача», «Работа», «Покупки», «Сделать», «Почта». "
        "Хорошо: «Инбокс Gmail», «Счёт МТС», «Деплой Рефкул». "
        "Не целое предложение; детали — только в description.\n"
